"""
The toolkit running METAL internally
Initialize:
    mtk = METAL_toolkit.METALToolkit(ethnicity = ["EU", "AF", "HISP"],
                                     verbose = 1,
                                     metal_path = "/mnt/mfs/cluster/bin/METAL/metal"
                                     )
        ethnicity: the name of ethnicities need to be analyzed
        verbose: bool, if True, when internally reading or writing files, the UI will
                 print a log
...
One-line Pipeline:
    ...
    it will create another header row, nothing informational.
...
""", formatter_class = argparse.RawDescriptionHelpFormatter)
    ...
    parser.add_argument("-P", '--metal_path', type = str, required = True,
                        help = """the path to METAL binary, the development environment cannot use
                                  "module load METAL" via Jupyterlab hold on SGE, you can either
                                    1. "module load METAL && echo $PATH" find the path
                                    2. modify generate_metal_running_script() method when your environment allows.""")
    ...
    parser.add_argument("-M", "--multi_line_header", required = False, action='store_true',
