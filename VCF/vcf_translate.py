...
class VCFTranslator():
    """Translate Coassin output to VCFv4.2 file format
    Initialize:
        vcft = VCFTranslator()
    Translate:
        vcft.write_vcf(input_path = "input_path",
                       bam_list = "*",
                       output_path = "output.vcf",
                       mode = "simplified",
                       format = "LPA"):
        input_path: str, the path of input folder with all the Coassin Pipeline
                    output inside the folder
        bam_list: list[str], if specified it will select the file in input_path
                  otherwise it will select all the folders
        ...
        format: str, in "VCF" mode it will list different genotypes together, or
                in traditional VCFv4.2 format
                in "LPA" mode it will list each individual genotypes on seperate
                lines, which is used as a encoding method
    """
    ...
    def _get_file(self, input_path, bam_name):
        '''
    ...
    def _data_load_wrapper(self, input_path, bam_list):
        """
        ...
        return pd.concat([
            self._data_cleaning(
                self._get_file(input_path = input_path,
                         bam_name = bam_name)
            )
    ...
    def _generate_meta(self,
        ...
        meta_information = textwrap.dedent(f'''\
    ...
    def _generate_header(self, sample_id_list):
        """
    ...
    def _new_index_VCF(self, l, i, mode):
        """
    ...
    def _new_index_LPA(self, l, i, mode, order):
        """
    ...
    def _assign_order(self, l):
        ...
        l.sort(key = self._comp_list)
        return [[j,i] for i,j in enumerate(l)]


    def _generate_line(self, pos, REF, ALT, sample_literal, mode, format):
        """
            ...
            if repeat == 1:
                ...
                return "\n".join(
                    ["\t".join(
                        ...
                        [self._new_index_VCF(l,i,mode) for l in sample_literal])
        ...
        # the genotype will use 1 in the literal and list them on the corresponding line
            ...
            order = self._assign_order([l2[0] for l1 in sample_literal for l2 in l1 if len(l2) == 2])
            ...
            return "\n".join(
                ["\t".join(
                    ...
                    self._formatting_lines(sample_literal,i,mode,order))
                 for i in range(repeat)
                ]
            )+"\n"


    def _formatting_lines(self, sample_literal,i,mode,order):
        #convert the lines into string
        body = str([self._new_index_LPA(l,i,mode,order) for l in sample_literal])
    ...
    def _get_positional_data(self, df):
        """
    ...
    def _formatting_alt(self, l):
        if (len(l) == 2 and l[1] == l[0]):
            return [l[0]]
        else:
            return sorted(l)

    def _get_genotype_literal(self, row, ref, alt_list, format):
        '''
        ...
        if format == "VCF":
            ...
            return [self._formatting_alt(alt),f"{gt_dict[major]}/{gt_dict[minor]}"]

    def _get_genotype(self, df, ref, alt_list, format):
        """
        ...
        return dict(
            df.apply(
                func = (
                lambda x: self._get_genotype_literal(
                            row = x,
                            ref = ref,
                            alt_list = alt_list,
                            format = format
    ...
    def _extract_data(self, df, ref, alt_list, TC, mode, format):
        """
        ...
        GT = self._get_genotype(df = df, ref = ref, alt_list = alt_list, format = format)
    ...
    def _get_sample_data(self, df_group_by_id, SampleID, ref, alt_list, TC, mode, format):
        """
    ...
    def write_vcf(self,
        ...
        df = self._data_load_wrapper(input_path, bam_list)

        # Write the output path
        self.write(path = output_path, content = self._generate_meta(mode = mode))
        ...
        # append the header
        self._append(path = output_path,
               content = self._generate_header(sample_id_list = sample_id_sorted_list))
        ...
        for pos in pos_sorted_list:
            ...
            REF, ALT, TC, alt_list = self._get_positional_data(df_group_pos)
            ...
            sample_literal = [self._get_sample_data(
