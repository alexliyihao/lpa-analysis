...
#SBATCH --job-name=lpa_analysis_slurm

# This sets the task range in the array
#SBATCH --array 1-3917:1
...
echo "Setting: Current Working directory is $PWD, current task id is $SLURM_ARRAY_TASK_ID"
...
PROJECT_SUBDIR="pipeline_output_slurm"
...
echo "Setting: working on ${INPUTFILES[$SLURM_ARRAY_TASK_ID - 1]}"

# Pull data file name from list defined above according to job id
INPUTFILENAME="$INPUTPATH/${INPUTFILES[$SLURM_ARRAY_TASK_ID - 1]}"

# Deal with the additional /hispanic, preventing additional folders created
FILENAME_CLEANED="`echo "${INPUTFILES[$SLURM_ARRAY_TASK_ID - 1]}" | sed 's/hispanic\///g'`"
echo "Setting: For input $INPUTFILENAME, cleaned into $FILENAME_CLEANED"

# Specify working directory under the /tempdata
WORKING_DIR="$PROJECT_SUBDIR/tempdata/$PROJECT_SUBDIR-$SLURM_ARRAY_TASK_ID"
...
DESTINATION_DIR="$PROJECT_SUBDIR/${INPUTFILES[$SLURM_ARRAY_TASK_ID - 1]}"
...
# Delete the tar of necessary data
